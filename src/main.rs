...
fn translate(text: &str, target_lang: &str, source_lang: Option<&String>, formality: Option<&String>)-> Result<()>  {
    ...
    let res = client.post("https://api.deepl.com/v2/translate")
        .multipart(form)
        .send()?;
    let json = res.json::<TranslationAnswer>()?;
    for translation in json.translations {
        println!("{} (from {})", translation.text, translation.detected_source_language);
    }
    Ok(())
}
...
fn main() {
    ...
    if args.len() < 3 {
        println!("Not enough argument, usage: {} TEXT TARGET_LANG [SOURCE_LANG] [more/less (FORMALITY)]", &args[0]);
        exit(1);
    }
